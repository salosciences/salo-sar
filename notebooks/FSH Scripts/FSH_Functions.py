...
def auto_tree_height_single_ISCE(directory, date1, date2, numLooks, noiselevel, flag_grad):
    ...
    xmlfile = subprocess.getoutput('find '+os.path.join(directory +'/int_'+date1+'_'+date2) + '/ -name *Proc.xml')
    tree = ET.parse(xmlfile)
    root = tree.getroot()
    root_tag = root.tag
    ...
    # decorrelation can occur due to processor and thermal noise, differential geometric, volumetric scattering, temporal changes,
    # Rotation of viewing geometry and Random motion over time
    # Operations

    # AMP and Coherence files need to be multilooked by a factor of 2 
    # We square it here to make sure all amplitude values are positive 
    inty1 = np.power(inty1, 2)           # Hardcoded based on 2 range looks and 10 azimuth looks
    inty2 = np.power(inty2, 2)  # Each one of these amplitude files is the backscatter amplitude of each satellite pass. Coherence is the Inty1 *inty2 over the sqrt of inty1**2 * Inty2**2
    # Here, all values below 0 we set to NaN because we do not want to transform them.
    ...
    # Formula to decorrelate thermal coupling from SAR atena, This is added from the radar electronics. 
    if noiselevel == 0.0:
        if root_tag[0] == 'i':
            ####### ALOS thermal noise level (insarApp)
            N1 = 55.5**2 # Squared because amplitude was also squared. Noise is added to the system because
            N2 = 55.5**2
        elif root_tag[0] == 's':
    ...
    corr_mag = remove_corr_bias(corr_mag,numLooks) # Corr_bias is used to re-estimate the actual correlation 
    corr_mag[corr_mag<0] = 0
    ...
    # Baseline formula to remove decorrelation... This is a determinent of how close each spacecraft flight was to one another. the closer the baseline the larger the sameness of each amplitude
    ...
    if flag_grad == 1:
        ...
        corr_vs[corr_vs>1] = 1
        corr_vs[corr_vs<0] = 0
    # We are now left with just the Volumetric correlation which we can use to estimate forest stand height. 
...
def remove_corr_bias(C,L):
    ...
    YC = set_interp(C) # Corrects coherence values for sameness based on number of looks
    plt.plot(m, D, 'o', C, YC, '-')
    return YC
